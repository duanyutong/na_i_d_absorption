function number_density1, r
; number of absorbing atoms in unit volume. can be a function of l
  number_density = r^0
  return, number_density
end

pro gas_model_v1, VDT, SUPPRESS_GFX=suppress_gfx
  ;+
  ;simple model for a parcel of gas
  ;-
  ...
  I_lambda0 = I0		; use a function instead for a radiation profile that
  						; depends on wavelength. eg. blackbody
  ...
  print, 'limits of lambda as defined by the velocity distribution' +$
	  'for transition1 is' $
  ...
  ; column density
  column_density = density_coefficient*qsimp('number_density', $
	  r_min, r_max, /double)
  ; optical depth
  tau1_lambda = s1_lambda*column_density	; optical depth for a specific
  											; wavelength, lambda
  tau2_lambda = s2_lambda*column_density
  tau0_lambda = s0_lambda*column_density
  ...
  if keyword_set(suppress_gfx) then begin
    ...
    ; for transitions of two types of atoms with
	; different veolocity distributions
